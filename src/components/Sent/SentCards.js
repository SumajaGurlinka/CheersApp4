...
const SentCards=(props)=> {
  ...
  const { user,getSentCardsAsync ,sentcards} = props;
  ...
  useEffect(() => {
    handleSent();
  }, [ getSentCardsAsync,search,rowsPerPage, page,sortOrder,orderBy ]);
...
{selectedButton==="All"&&(
          ...
          {sentcards?.sentcardsData.length === 0 ? (
            <TableBody>
              <TableRow>
 ...
 {sentcards?.sentcardsData && sentcards?.sentcardsData
        ...
        <TablePagination
          ...
          component="div"
          count={sentcards?.sentcardsData.length}
